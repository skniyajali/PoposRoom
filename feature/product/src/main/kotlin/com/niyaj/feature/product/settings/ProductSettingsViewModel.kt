...
import com.niyaj.feature.product.settings.ProductSettingsEvent.GetExportedProduct
...
import com.niyaj.feature.product.settings.ProductSettingsEvent.OnIncreaseProductPrice
...
class ProductSettingsViewModel @Inject constructor(
    ...
    fun onEvent(event: ProductSettingsEvent) {
        when (event) {
            ...
            is OnChangeProductPrice -> {
                _productPrice.intValue = event.price.safeInt()
            }

            is OnIncreaseProductPrice -> {
                viewModelScope.launch {
                    val data = if (mSelectedItems.isNotEmpty()) {
